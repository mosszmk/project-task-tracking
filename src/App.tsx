...
export function App() {
  ...
  // 1. Initial Load: Load from local cache, ensuring ONLY MAKRO PACK 3 and all 12 tasks are present
  useEffect(() => {
    const cached = loadLocalCache();
    const hasOnlyMakro = 
      cached?.projects?.length === 1 && 
      cached.projects.some((p) => p.id === 'proj-makro-1' || p.name.includes('MAKRO PACK 3'));
    const hasValidTasks = (cached?.tasks?.length ?? 0) >= 10;

    if (cached && hasOnlyMakro && hasValidTasks) {
    ...
    if (getAppsScriptUrl()) {
      ...
      fetchFromGoogleSheet()
        .then((res) => {
          if (res?.status === 'success' && res.data) {
            const newProjects = (res.data.projects && res.data.projects.length > 0) ? res.data.projects : mockProjects;
            const newTasks = (res.data.tasks && res.data.tasks.length > 0) ? res.data.tasks : mockTasks;
            const newProcurements = res.data.procurements || mockProcurements;

            setProjects(newProjects);
            setTasks(newTasks);
            setProcurements(newProcurements);
            saveLocalCache(newProjects, newTasks, newProcurements);
      ...
      // Auto-refresh when tab is focused (keeps all team members synced across different PCs)
      const handleWindowFocus = () => {
        fetchFromGoogleSheet()
          .then((res) => {
            if (res?.status === 'success' && res.data) {
              if (res.data.projects && res.data.projects.length > 0) setProjects(res.data.projects);
              if (res.data.tasks && res.data.tasks.length > 0) setTasks(res.data.tasks);
